grid_in_string = """08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
    ...
    for idx_row in range(height):
        for idx_col in range(length):
            ...
            for diff in range(CONTINUOUS):
                product *= grid[idx_row][idx_col+diff]
    ...
    for idx_col in range(length):
        for idx_row in range(height):
            ...
            if idx_row + CONTINUOUS > length:
                ...
                break
            for diff in range(CONTINUOUS):
                product *= grid[idx_row+diff][idx_col]
            if product > max_product:
                max_product = product
                print "%9d %2d %2d %2d" % (
                    product, idx_row, idx_col, grid[idx_row][idx_col])
